...
sys_exofork(void)
{
	...
	// The parent returns the child's envid.
	return e->env_id;
}
...
sys_env_set_trapframe(envid_t envid, struct Trapframe *tf)
{
	...
	r = user_mem_check(e, tf, sizeof(struct Trapframe), PTE_U);
	if (r < 0) return r;
...
sys_page_alloc(envid_t envid, void *va, int perm)
{
	...
	r = page_insert(e->env_pgdir, pi, va, perm);
	if (r < 0) {
		page_free(pi);
		return r;
	}
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	r = envid2env(srcenvid, &es, true);
	if (r < 0) {
		return r;
	}
	r = envid2env(dstenvid, &ed, true);
	if (r < 0) {
		return r;
	}
	...
	pte_t *pte;
	struct PageInfo *pi = page_lookup(es->env_pgdir, srcva, &pte);
	if (pi == NULL) {
		return -E_INVAL;
	}
	...
	r = page_insert(ed->env_pgdir, pi, dstva, perm);
	if (r < 0) {
		return r;
	}
...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
	...
	if (srcva < (void *)UTOP) {
		...
		pte_t *pte;
		struct PageInfo *pi = page_lookup(curenv->env_pgdir, srcva, &pte);
		if (pi == NULL) {
			return -E_INVAL;
		}
		...
		if (e->env_ipc_dstva < (void *)UTOP) {
			r = page_insert(e->env_pgdir, pi, e->env_ipc_dstva, perm);
			if (r < 0) {
				return r;
			}
		}
